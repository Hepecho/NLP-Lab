...
class Model(object):
    def __init__(self, vocab_size, out_size, word_emb, opt):
        """功能：对LSTM的模型进行训练与测试
           参数:
            vocab_size:词典大小
            out_size:标注种类
        ...
        # 加载模型参数
        self.emb_size = TrainingConfig.emb_size
        self.opt = opt
        if opt.model == 'cnn':
            self.hidden_size = CNNConfig.hidden_size
        else:
            self.hidden_size = LSTMConfig.hidden_size
        # 根据是否添加crf初始化不同的模型 选择不一样的损失计算函数
        if not opt.crf:
            self.model = BaseModel(word_emb, vocab_size, self.emb_size,
                                self.hidden_size, out_size, opt).to(self.device)
            self.cal_loss_func = cal_loss
        else:
            self.model = CRF(word_emb, vocab_size, self.emb_size,
                                    self.hidden_size, out_size, opt).to(self.device)
    ...
    def train(self, word_lists, tag_lists,
        ...
        for e in range(1, self.epoches+1):
            ...
            cache['dev_loss'].append(val_loss)
        return cache
...
class CRF(nn.Module):
    def __init__(self, word_emb, vocab_size, emb_size, hidden_size, out_size, opt):
        ...
        self.base_model = BaseModel(word_emb, vocab_size, emb_size, hidden_size, out_size, opt)
    ...
    def forward(self, sents_tensor, lengths):
        # [B, L, out_size]
        emission = self.base_model(sents_tensor, lengths)
